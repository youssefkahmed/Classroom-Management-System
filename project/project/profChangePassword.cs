...
namespace project
{
    public partial class profChangePassword : UserControl
    {
        OracleConnection conn = new OracleConnection(regProfessor.ordb);
        public profChangePassword()
        {
            InitializeComponent();
        }
        ...
        private void bunifuThinButton21_Click(object sender, EventArgs e)
        {
            ...
            if (notnull_validate()&&pass_validate()) {
                ...
                c.Connection = conn;
                c.CommandText = "select member_password from staff where member_id= :m_id";
                        ...
                        if (String.Equals(new_pass.Text.ToString(), Cnew_pass.Text.ToString()))
                        {
                           // MessageBox.Show("enteredddd");
                            ...
                            if (x != -1)
                            {
                                ...
                                c3.Connection = conn;
                                c3.CommandText = "update staff set member_password =:pass where member_email = :mail";
        ...
        private void profChangePassword_Load(object sender, EventArgs e)
        {
            conn.Open();
        }
